...
func UploadAvatar(c *gin.Context) {
	...
	claims, _ := util2.ParseToken(c.GetHeader("Authorization")) //解析token，登录保护！！！！！！
	//没有token会出错
...
func SendEmail(c *gin.Context) {
	...
	claims, _ := util2.ParseToken(c.GetHeader("Authorization")) //解析请求header里面的token，登录保护！！！！！！
	//没有token会出错
...
func DisplayAmount(c *gin.Context) {
	...
	//判断登录状态
	claims, _ := util2.ParseToken(c.GetHeader("Authorization")) //未初始化要加':'
