...
import { Image,Card, Divider,List,Collapse} from 'antd';
import api from "../../../api"
import "./index.css"
...
export default function Admin(){
    ...
    // 获取将要审批的全部生产方商品信息
    const  getAllOrders = ()=>{
        api.getitemListApproved().then((res)=>{
    ...
    return (
        <div>
            <h1 style={{fontSize: "20px",letterSpacing:"4px",padding:"10px 0px 5px 10px" }}>商品审批通过列表：</h1>
             ...
             renderItem={(item) => (
                <List.Item>
                    <Card title={"商品编号:"+item.id} style={{textAlign:"center"}}>
                        ...
                        <div ><strong>商品名:</strong>{item.goodsName}</div>
                        <div ><strong>总件数:</strong>{item.goodsNum}件</div>
                        <div ><strong>供货商:</strong>{item.supplierName}</div>
                        <div ><strong>单价(元):</strong><em>¥</em>{item.price.toFixed(2)}</div>
