  ...
  const orderContent = (
      ...
      <div className="block">
          ...
          {  
           SecondTransferInfo.map((res)=>(
              ...
              <Fragment key={uuidv4()}> 
                    <li key={uuidv4()}> {res.category===0 ?"订单生成":"订单转让"} &nbsp;&nbsp;({res.level})  &nbsp;&nbsp;  &nbsp;&nbsp;{res.time}</li>
    ...
    const firstOrderTransfer = (orderId)=>{
        api.getfirstOrderList(orderId).then(res=>{
          ...
          {
            api.patchfirstOrderList1(orderId).then((res)=>{
              getAllOrders();
    ...
    const columns = [
      {
        ...
        render: (itemdetail) =>(<div><div> 
          ...
          <div  className="fl"  style={{transform:"scale(0.7)"}}>订单生成时间：{itemdetail.time}</div>
          <div  className="fl"  style={{transform:"scale(0.7)"}}>订单批次号：{itemdetail.orderId}</div>
          </div>),
        align:'center',
      },
      {
