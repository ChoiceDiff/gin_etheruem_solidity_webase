...
export default function ItemOrder(){
  ...
  const showBlockInfo = (obj)=>{
    ...
    const getAllOrders=()=>{
        api.getsecondOrderListOrderState()
        .then((res)=>{
          ...
          api.getblockChain().then(res=>{
              ...
              let prevBlockHash=chainInfo[chainInfo.length-1].id;
                  ...
                  api.postblockChain({
                    ...
                    // 生成新的二级订单后将原来的二级订单删除
                    console.log(obj.id)
                     api.deletesecondOrderList(obj.id).then(()=>{
                      getAllOrders();
        ...
        renderItem={(item,index) => (
          <List.Item>
            <Card title={"批次号:"+item.orderId} key={index}>
              ...
              {/* <img src={require("../../../"+item.imgUrl)}  className="fl" style={{display:"inline-block", width:"35%",aspectRatio:"1/1"}} /> */}
              ...
              <div>
               ...
               <div className="fl express" style={{display:dynamicComparison(item.orderId,item.price )?"none":""}}>
                <strong>动态折扣价:</strong>
                <em>¥</em>
                {(discout.length===0)?"":(discout.find((res)=>{ return (res.orderId==item.orderId)}).price*item.goodsNum).toFixed(2)}
              </div>
