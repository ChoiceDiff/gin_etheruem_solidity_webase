...
// 加载查询区块信息
  ...
  //转让一级订单
  const firstOrderTransfer = (orderId)=>{
    api.getfirstOrderList(orderId).then(res=>{
      if(res.data.data.order_state===1)
        message.info("订单正在转让中")
      else if(res.data.data.deal_state===1)
        message.info("商品发布中，不可转让订单")

      else{
        api.patchfirstOrderList1(orderId)
        .then((res)=>{
            console.log(res)
            getAllOrders();
            message.success("转让一级订单")
        })}
    })
  }
  // 二级商品发布
  const secondOrderItem = (orderId)=>{
    
    let checkTime=new Date((new Date).getTime() + 8*60*60*1000).toJSON().split('T').join(' ').substr(0,19);
    api.getfirstOrderList(orderId).then((res)=>{
      
      if(res.data.data.order_state===1){
        message.info("订单转让中,不可发布商品");
      }
      else if(res.data.data.deal_state===1)
      {
        message.info("商品发布中");
      }else
      {
            ...
            let time =Date.now();
            data.id = '0x'+sha256(time.toString());
            // data.orderId=checkTime.substr(2,8).split('-').join('')+Date.now().toString().substr(-8,8);
            data.orderId=data.orderId;
            data.time = checkTime;
            data.order_state=1;
            data.deal_state=0;
            data.firstOrderId= orderId;
            api.postsecondOrderList(data).then((res)=>{
                api.patchfirstOrderList2(orderId).then((res)=>{
                  getAllOrders();
                  message.success("订单商品发布成功");
                })
  ...
  const showOrderTransfer = (id)=>{
      api.getfirstOrderList(id).then(response=>{
    ...
    const columns = [
      {
        ...
        <div  className="fl"  style={{transform:"scale(0.7)",marginLeft:"-20px"}}>订单生成时间：{itemdetail.time}</div>
        <div  className="fl"  style={{transform:"scale(0.7)",marginLeft:"-10px"}}>订单批次号：{itemdetail.orderId}</div>
        </div>),
        align:'center',
      },
      {
        ...
        render: (applytype,record) => (
          <Space size="middle">
            {applytype.order_state===0?
              (applytype.deal_state===0?<Tag color='green' key={record}>申购成功</Tag>:<Tag color='yellow' key={record}>商品发布中（即将产生二级订单）</Tag>): 
              (<Tag color='blue' key={record}>一级订单转让中</Tag>)
            }      
          </Space>
        ),
        align:'center',
        ...
        render: (id, record) => (
          <Space size="middle" direction="vertical">
           ...
           >
            ...
            <Button onClick={()=>firstOrderTransfer(id)}><strong>一级订单转让</strong></Button>
            <Button onClick={()=>secondOrderItem(id)}><strong>商品发布</strong></Button>
          </Space>
        ),
        align:'center',
      },
